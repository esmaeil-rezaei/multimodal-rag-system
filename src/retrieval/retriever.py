...
class Retriever:
    ...
    def retrieve(
        ...
        It:
        - pulls initial candidates
        - optionally expands them (parents / sentence window)
        - reranks them
        - orders them for the LLM
        - trims them to fit the context limit
        """
    ...
    def _compress_llm_lingua(self, text: str) -> str:
        """
        Compress text using an LLM while keeping the important parts.
        """
    ...
    def _compress_extractive(text: str, max_sentences: int = 3) -> str:
        """
        Cheap fallback compression — just keeps the first few sentences.
        """
        import re
