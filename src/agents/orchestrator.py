...
def _reconcile_preferences(
    new_pref: str, existing: list[str], openai_client
) -> list[str]:
    if not existing:
        return [new_pref]
...
class RAGOrchestrator:
    ...
    def __init__(self, container: AppContainer) -> None:
        ...
        self._fairness_sample_rate: float = (
            _cfg.evaluation.get("fairness", {}).get("live_sample_rate", 0.05)
        )
    ...
    async def run(
        ...
        if routing_intent in ("retrieval", "conversational"):
            ...
            if detection.clear_all:
                ...
                if detection.follow_on_query:
                    ...
                    logger.info("Mixed message — routing question '%s' to retrieval.", effective_query[:80])
                elif detection.redo:
                    last_question = next(
                        (m["content"] for m in reversed(conversation_history or [])
                         if m.get("role") == "user"),
                        None,
                    )
                    if last_question:
        ...
        if extracted_pref == "__CLEARED__":
            history_with_pref = history_with_pref + [
                {"role": "system", "content": "[USER PREFERENCE RESET] All previous preferences have been cleared."}
            ]
        elif extracted_pref:
            history_with_pref = history_with_pref + [
